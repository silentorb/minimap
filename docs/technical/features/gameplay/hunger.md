...
- CompuQuest accessories:
  ...
  - **`movement_energy`** — `activation.kind: none`; not `player_selectable`; effect `drain_resource_by_distance` (energy; **`unitsPerAmount` 120** → **1** energy per **120** world units). Tracks last `Character.Position`; first tick seeds without drain.
  - **`eat`** — modal; optional **`enabledWhen`** `{ "id": "food", "atLeast": 1 }`; `modify_resource_on_use` (+5 energy) with use cost **1 food**.
- Character defs (`generic`, `zombie`, …) list **`energy_upkeep`**, **`movement_energy`**, and **`eat`** in accessories.
...
- Passive tick: Simulation contract such as **`IPassiveEffect`**; **`GameWorld`** ticks it for characters (`drain_resource`, `drain_resource_by_distance`, `modify_resource_by_ratio_bands`). Cell-actor grow / world-passive tick remains separate.
