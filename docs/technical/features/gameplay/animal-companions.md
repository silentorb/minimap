...
- **`GameWorld.TrySpawnNearbyActor`** — spawn one AI character on nearby grass (then all-grass fallback) with caller-chosen faction / aggression / `seekCrops`. **`TrySpawnNearbyHostile`** remains a thin wrapper that passes the rival faction id.
- CompuQuest effect **`spawn_nearby_ally`** (`SpawnNearbyAllyEffect`):
  ...
  - On tick: if not yet spawned, resolve `characterId` via **`GameWorld.TryGetActorDefinition`** (missing → fail-fast); call **`TrySpawnNearbyActor`** with the owner’s **`FactionId`** and owner axial from **`HexWorldLayout.WorldToAxial`**; on success mark done; on null retry later.
