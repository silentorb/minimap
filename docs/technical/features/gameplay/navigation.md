...
Simulation-owned movement steering for AI floor goals. Implements game [ai.md](../../../game/features/gameplay/ai.md). Related: [controllers.md](controllers.md).
...
- **Minimap.Client** (`WorldApp`) constructs the navigation host, parents it under the world scene, **rebuilds** the mesh on session bind and level regeneration, and **upgrades** each `AiController` from Direct to Godot crowd steering (including mid-wave spawns).
