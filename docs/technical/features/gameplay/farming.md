...
- **`harvest`**: mature grow with no emerge id → remove actor + grant yield; mature grow with `emergeCharacterId` → `TryEmerge` (no yield). Uses `GameWorld.SpawnChaseCharacter` + `RivalFactionId`.
- Free-loot actor **`loose_carrot`**: accessory with `pickup_resource` (`IDefaultInteractionEffect`) → food ×1.
