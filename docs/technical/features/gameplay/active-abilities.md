...
- **`PlayerController`**: feeds primary/secondary fire held, modal select, ability activate/back edges, environment interact; owns preview state for modal abilities that use two-step activate (e.g. `ICellPlacementEffect`); for modals with **`IInstantUseEffect`** (no placement), activates immediately (pay cost + apply).
