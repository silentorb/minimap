...
- **`AbilityLoadout`** on **`Character`**: dedicated bind map + unbounded modal list; rebuilt on accessory add/remove and when enable state flips. Rebuild includes only **enabled** accessories with **`dedicated`** / **`modal`** activation (**`none`** is never equipped). Rebuild **skips disabled** accessories; preserves selection by accessory id when still present; if the previous selection dropped, selects the **next** remaining modal (then wrap/clamp to first if none after). **Unequipped** is represented as `SelectedModalIndex == -1` (`SelectedModal` null) even when the modal pool is non-empty. Rebuild preserves unequipped when that was the previous selection. **`CycleModal(delta)`** wraps through the modal pool **plus** one unequipped slot (`count + 1`); no-op when the pool is empty. First rebuild with modals present still defaults to index `0` (first modal); unequip is explicit via cycle.
