...
Client UI and local session wiring for player status. Implements [../../game/features/player-hud.md](../../game/features/player-hud.md). Related: [controllers.md](controllers.md), [characters-and-factions.md](characters-and-factions.md), [resources.md](resources.md), [ui-icons.md](ui-icons.md), [active-abilities.md](active-abilities.md), [hunger.md](hunger.md).
...
- **Minimap.Client** (`ClientSession.BuildHudModels`) maps `PlayerController` pawns + `GameContent.Resources` → `PlayerHudModel` (including selected modal definition icon/name from the filtered loadout) and `WorldApp` pushes snapshots into `PlayerHudPanel`.
