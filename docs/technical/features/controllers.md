...
- **Minimap.Simulation** owns `GameSession` (world create, scenario tick, human pawns, game-over). **Minimap.Client** (`ClientSession` / `WorldApp`) attaches `PlayerController`s to unpossessed human pawns, feeds per-player move and aim input via **`LocalInputAggregator`**, and wires Godot navigation steering for AI (see [navigation.md](navigation.md)).
