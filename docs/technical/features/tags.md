...
Runtime tag ids for classifying definitions and content. Related: [accessories.md](accessories.md), [resources.md](resources.md), [domains.md](domains.md), [extensions.md](extensions.md), [definition-config.md](definition-config.md).
...
- Shipped tags include **`player_selectable`** (CompuQuest; accessory filter), **domain ids** (`gardening`, `computing`; see [domains.md](domains.md)), and **resource type ids** (`health`, `max_health`, `electronics`, `seeds`, `ammo`, …) resolved when loading resource definitions.
