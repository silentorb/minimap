...
- **`ICellPlacementEffect`** (Simulation): `CanPlace` / `TryPlace` for accessory effects. CompuQuest **`PlaceRandomObjectEffect`** (`place_random_object`) picks from a pool of placed-object ids registered on the world.
