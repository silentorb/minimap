...
- **Accessory UI**: `AccessorySelectionPanel` under each panel’s `CustomizeArea`; visible/interactive only in **Claimed**. Choices persist Claimed↔Ready; cleared on Available. State model (`LobbyAccessorySelectionState`) tracks **prior-owned** (locked, cannot return) and **stage-owned** (can take/return). Available list is catalog minus all owned. Panel shows remaining points available.
