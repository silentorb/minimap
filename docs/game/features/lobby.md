...
- Accessory selection UI (Claimed only): available icon grid, description of the focused accessory, owned icon grid. Budget comes from core **`player.accessoryPoints`** (default 2). Catalog comes from the integrator’s player-selectable accessories.
- Input while Claimed: arrows / D-pad / stick navigate grids; **Space** / **A** take or return the focused accessory (if points allow); **Enter** / **Start** ready up; **Escape** / **B** back (Ready→Claimed keeps choices; Claimed→Available clears selection and devices).
