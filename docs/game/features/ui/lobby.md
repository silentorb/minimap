...
- Each panel shows **Back** / **Forward** buttons at the bottom when the current step can navigate earlier or later in the wizard (same transitions as Escape/B and Enter/Start/A on profile). Available has neither; Ready has Back only; Forward is disabled when profile confirm is impossible. While **Selecting profile** or **Ready**, Back/Forward are focusable (pad may D-pad to them). While **Selecting accessories**, those buttons stay unfocusable so D-pad stays on the accessory grids; use **B** / **Start** instead.
...
- Input while Selecting profile: ←/→ cycle; **A** / **Enter** / **Start** confirm profile; **Escape** / **B** back to Available (clears devices and selection).
