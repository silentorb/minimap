...
Faction membership and hostility. Related: [ai.md](ai.md), [combat.md](combat.md), [damage.md](damage.md). Technical: [characters-and-factions.md](../../../technical/features/gameplay/characters-and-factions.md).
...
- **Current game mode (surface only)**: two factions with ids **1** (player’s faction) and **2** (rival). Spawn:
  ...
  - **Rival AI**: intrinsic **zombie spawners** emit zombies / zombie farmers on faction **2** (see [waves.md](waves.md)); the wave/level clock paces level regen but does not itself burst-spawn rivals in normal play; **crazed carrots** that emerge from farming also use rival id **2** (allied with zombies, hostile to faction 1)
