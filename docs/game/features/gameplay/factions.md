...
- **Current game mode (surface only)**: two factions with ids **1** (player’s faction) and **2** (rival). Spawn:
  ...
  - **Rival AI**: intrinsic **zombie spawners** emit zombies / zombie farmers on faction **2** (see [waves.md](waves.md)); **crazed carrots** that emerge from farming also use rival id **2** (allied with zombies, hostile to faction 1)
