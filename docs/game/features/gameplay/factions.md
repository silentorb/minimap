...
- **Current game mode (surface only)**: two factions with ids **1** (player’s faction) and **2** (rival). Spawn:
  ...
  - **Rival AI**: none from wave spawners in normal sandbox play (spawners / waves parked; see [waves.md](waves.md)); **crazed carrots** that emerge from farming still use rival id **2** (allied with zombies, hostile to faction 1)
