...
- **Cell-anchored placeables** (CompuQuest): computers start with max/start health **40**; planted vegetables (carrot_growing / corn_growing / melon_growing) with **25**; **zombie spawners** with **400**. Starting amounts come from actor definition `resources`.
...
- **Level transition** (after all wave intervals in a level): all human players are **healed to max health** and **resurrected** if they had died during the level. See [scenarios.md](../session/scenarios.md).
