...
- Modal activate: gamepad **`JoyButton.X`** / keyboard **Space**. Some abilities activate immediately (**instant use**, e.g. Eat, Heal self); others use a two-step **preview → confirm** flow (e.g. cell placement). This is general, not plant- or placement-only. Activate is a no-op when **none** is selected.
