...
- **Fire direction**:
  - **Player**: aim via right stick / mouse; fire only while **primary fire** is held (RT / LMB). Non-zero aim updates character **facing**; move does not. If aim is zero, fall back to last facing. Gun is a **dedicated** ability bind (`primary_fire`), not modal.
